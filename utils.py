...
def decode_boundary(outputs, entities, length):
    class Node:
        def __init__(self):
            self.THW = []                # [(tail, type)]
            self.NNW = defaultdict(set)   # {(head,tail): {next_index}}
    ...
    for instance, ent_set, l in zip(outputs, entities, length):
        candidates = []
        for ir, row in enumerate(instance[:l,:l]):
            for ic, col in enumerate(row):
                if col != 0 and col < 7:
                    if ic == ir:
                        candidates.append(([ic], col))
                    elif ir > ic:
                        candidates.append(([ic, ir], col))
        
        predicts = set([convert_index_to_text(list(range(x[0][0],x[0][-1]+1)), x[1]) for x in candidates])
        for x in predicts:
            index, type = x.split("-#-")
            assert len(index) > 0
        decode_entities.append([convert_text_to_index(x) for x in predicts])
